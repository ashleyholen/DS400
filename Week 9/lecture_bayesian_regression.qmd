...
```{r}
ggplot(data = bikes, aes(x = month, y = rides)) + 
  geom_col()
```
...
```{r}
ggplot(data = bikes, aes(x = temp_actual, y = rides, color = season, size = holiday)) + 
  geom_point() 
```

```{r}
ggplot(data = bikes, (aes(x = date, y = rides, color = weekend, shape = weekend)))+ 
      geom_point()
```
...
```{r}
# Bad Day
new_data_for_prediction_2 <- data.frame(
  temp_feel = 30, 
  temp_actual = 40,
  season = "winter",      
  year = 2011,           
...
```{r}
prediction <- 
  posterior_predict(bike_model_all, newdata = new_data_for_prediction_2)
```
...
```{r}
# Good Day
new_data_for_prediction_3 <- data.frame(
  temp_feel = 75, 
  temp_actual = 78,
  season = "summer",      
  year = 2011,           
...
```{r}
prediction <- 
  posterior_predict(bike_model_all, newdata = new_data_for_prediction_3)
```
