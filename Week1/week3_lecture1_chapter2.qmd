...
fake_news_limited <- fake_news_limited %>%
  group_by_all() %>%
  count()
```
...
```{r}
prior_probability_article_real <- 0.6
```
...
```{r}
prior_probability_article_has_excl <- 0.12
```
...
```{r}
prior_probability_article_no_excl <- 0.88
```
...
```{r}
likelihood_article_fake_has_excl <- 0.267
```

L(Bc\|A) = 0.022

```{r}
likelihood_article_real_has_excl <- 0.022
```
...
```{r}
(prior_probability_article_fake * likelihood_article_fake_has_excl) / prior_probability_article_has_excl
```
...
```{r}
colnames(pop_vs_soda)
```

```{r}
unique(pop_vs_soda$word_for_cola)
```
...
-   0.626 percent of total people do not say pop

-   0.374 percent of people say pop

```{r}
pop_total_percent <- 0.374
no_pop_total_percent <- 0.626
```
...
```{r}
south_prior_probability <- 0.241
midwest_prior_probability <- 0.388
northeast_prior_probability <- 0.206
west_prior_probability <- 0.165
```
...
```{r}
likelihood_midwest_pop <- 0.645
likelihood_northeast_pop <- 0.273
likelihood_south_pop <- 0.079
liklihood_west_pop <- 0.294
```

```{r}
(south_prior_probability * likelihood_south_pop) / pop_total_percent
```

-   posterior probability model determining the probability that the person is from the south based on using the word "pop" is about 5.1%
...
1\) Prior Probability (census data)
