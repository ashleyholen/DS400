...
```{r}
ggplot(bikes_scooters, aes(x = temp_actual, y = scooter_rides, color = day_of_week, size = weather_cat)) + 
  geom_point()
```

```{r}
ggplot(bikes_scooters, aes(x = day_of_week, y = scooter_rides, fill = weather_cat)) + 
  geom_col()
```
...
ggplot(data = avg_scooter_rides_by_day, aes(x = day_of_week, y = mean_rides)) + 
